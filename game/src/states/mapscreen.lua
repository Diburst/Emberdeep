...
-- THE CLIP IS THE ONE THING ON SCREEN THAT DOES NOT LIVE IN LOGICAL
-- UNITS.
--
-- Every draw call in the game works in 480x270 because love.draw pushes
-- one scale(G.RS) over the whole frame. setScissor is the exception: it
-- takes DRAWABLE PIXELS and ignores the transform stack entirely, so the
-- same numbers that place the view rectangle cut a hole G.RS times too
-- small in the corner of the canvas. With the shipped renderscale of 4
-- that hole is logical (1.5,5)-(118.5,55.5), and the map -- links, rooms,
-- labels, crosshair, all of it -- was drawn outside it and thrown away.
-- The header printed before the clip and the footer after it, which is
-- exactly the "header and caption but no image" that was reported.
--
-- Scale going in, and nothing else here has to know.
local function clip(g, x, y, w, h)
  if not x then return g.setScissor() end
  local s = G.RS or 1
  g.setScissor(x * s, y * s, w * s, h * s)
end
...
-- input
-- ------------------------------------------------------------------
-- THE MAP OWNS THE GAME KEYS, SO IT CANNOT ALSO ANSWER TO THEM AS MENU
-- KEYS.
--
-- Its own footer says what its controls are: JUMP zooms in, FIRE zooms
-- out, UTIL fits, MAP closes. But `cancel` is generated by the letter J
-- (see the wasd table in input.lua) and J is the FIRE key -- so the one
-- press that is supposed to zoom out was closing the screen instead, and
-- there was no way to zoom out at all.
--
-- ESCAPE still cancels, because a player pressing escape on an overlay
-- means it. The test is on the KEY, not on the action, precisely because
-- the action is ambiguous and the key is not.
function S:menu(action, ev)
  local escaped = action == "cancel" and ev and ev.key == "escape"
  if escaped or action == "backbtn" or action == "start" then
    G.State.pop()
...
function S:draw()
  ...
  clip(g, VIEW.x, VIEW.y, VIEW.w, VIEW.h)
